...
class DemoApiService {
  ...
  private stepGuides: Record<string, StepGuide[]> = {
    'add-product': [
      ...
      },
      {
        ...
        element: "button",
        text: "Bulk Import",
        description: "For managing multiple products, you can also use the 'Bulk Import' feature to upload many products at once using CSV or Excel files",
    ...
    'bulk-import': [
      ...
      },
      {
        ...
        element: "button",
        text: "Bulk Import",
        description: "Click on the 'Bulk Import' button to upload multiple products at once using CSV or Excel files",
        selector: "[data-demo-element='bulk-import']"
      },
      {
        step_number: 3,
        ...
        description: "You can also organize your products by clicking 'Manage Categories' to set up product categories and subcategories",
    ...
    'setup-widget': [
      {
        ...
        element: "button",
        text: "Settings",
        description: "Click on the 'Settings' button in the navigation to access system configuration",
    ...
    'login': [
      {
        ...
        element: "button",
        text: "Login",
        description: "First, click on the 'Login' button to access the authentication system. This will open the login form where you can enter your credentials.",
        selector: "[data-demo-element='login-button']"
      },
      {
        step_number: 2,
  ...
  async sendMessage(message: string, mode: 'show' | 'tell' | 'do'): Promise<SendMessageResponse> {
    ...
    return {
      messageId: this.generateId(),
      response: defaultResponses[mode] || "I'm here to help! Try hovering over different elements on the page to see contextual assistance.",
      timestamp: new Date(),
      mode: mode
    };
  }
  ...
  private getElementDescription(elementType: string | null): string {
    ...
    return elementType ? descriptions[elementType] || 'Click to explore this feature' : 'Click to explore this feature';
  }
  ...
  private async handleStepGuideRequest(message: string, mode: 'show' | 'tell' | 'do'): Promise<SendMessageResponse> {
    const messageLower = message.toLowerCase();
    ...
    } else if (messageLower.includes('setup') || messageLower.includes('set up') || messageLower.includes('widget')) {
      guideType = 'setup-widget';
    } else if (messageLower.includes('login') || messageLower.includes('sign in') || messageLower.includes('log in')) {
      guideType = 'login';
    }
  ...
  private updateAutoStepIndicatorStatus(status: string, type: 'preparing' | 'executing' | 'completed'): void {
  ...
  private async executeLoginStepsAutomatically(steps: StepGuide[], currentIndex: number): Promise<void> {
    ...
    setTimeout(async () => {
      ...
      this.showIPhoneNotification(
        'Executing...',
        `Performing: ${currentStep.description}`,
        'info'
      );
      ...
      switch (step.action) {
        ...
        case 'fill':
          if (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement) {
            ...
            console.log('Auto-filled element:', step.text, 'with value:', this.getSampleValue(step));
          }
          break;
  ...
  private async typeTextWithAnimation(element: HTMLInputElement | HTMLTextAreaElement, text: string): Promise<void> {
  ...
  private getSampleValue(step: StepGuide): string {
    if (step.text?.toLowerCase().includes('email') || step.text?.toLowerCase().includes('username')) {
  ...
  private addStepClickHandlers(): void {
    ...
    console.log(`Current step ${this.currentStepIndex + 1}: Looking for element with selector:`, selector);
    ...
    if (element) {
      ...
      console.log(`Current step ${this.currentStepIndex + 1}: Click handler added with index:`, this.currentStepIndex);
    } else {
      console.warn(`Current step ${this.currentStepIndex + 1}: Element not found for selector:`, selector);
    }
  }
  ...
  private nextStep(): void {
    ...
    if (this.currentStepIndex < this.currentStepGuide.length - 1) {
      ...
      setTimeout(() => {
        ...
        if (this.currentStepIndex === 2 && this.currentStepGuide && this.currentStepGuide[this.currentStepIndex]?.action === 'fill') {
          this.showProductForm();
        }
        
        // Special handling for login form step
        if (this.currentStepIndex === 1 && this.currentStepGuide && this.currentStepGuide[this.currentStepIndex]?.action === 'fill') {
          this.showLoginForm();
        }
  ...
  private removeStepHighlights(): void {
    ...
    document.querySelectorAll('.step-indicator, .step-completed, .step-element-clicked').forEach(el => {
    ...
    document.querySelectorAll('[id*="step-"]').forEach(el => {
      if (el.id.includes('spotlight') || el.id.includes('border') || el.id.includes('description')) {
        el.remove();
      }
    });
    ...
    document.querySelectorAll('*').forEach(el => {
      ...
      if (element.style && element.style.animation && element.style.animation.includes('stepPulse')) {
  ...
  private showIPhoneNotification(title: string, message: string, type: 'success' | 'info' | 'warning' | 'error' = 'success'): void {
