/**
 * `Text` tests: resolves the color from `tone` (`inherit` included), and truncation/size/weight/align/line
 * height/block/italic each fold into the style object.
 */
...
describe('Text', () => {
  it('colors by tone', () => {
    const { container } = render(<Text tone='muted'>hi</Text>);
  ...
  it('inherits color when the tone is inherit', () => {
    const { container } = render(<Text tone='inherit'>hi</Text>);
