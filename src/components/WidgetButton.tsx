...
export const WidgetButton: React.FC<WidgetButtonProps> = ({
  ...
  const { getWidgetText, getActiveAvatar, getWidgetPosition, getWidgetSettings } = useWidgetAtmosphere(config);
  ...
  return (
    <div className={`fixed ${effectivePositionClasses} transition-all duration-500 ease-in-out ${showWelcomeText && !isOpen ? (effectivePosition.includes('left') ? 'transform translate-x-64' : 'transform -translate-x-64') : ''}`} style={{ zIndex: widgetPosition.z_index || 50 }}>
      <button
        onClick={onClick}
        className={`
      ...
      >
       ...
       {!isOpen && showWelcomeText && effectiveSettings.widget_appearance === 'default' && (
         <div className={`absolute ${effectivePosition.includes('left') ? 'right-16' : 'left-16'} top-1/2 transform -translate-y-1/2 px-4 py-3 bg-white text-gray-800 text-sm rounded-lg shadow-lg w-64 gradient-border ${effectivePosition.includes('left') ? 'animate-slide-in-right' : 'animate-slide-in-left'} cursor-pointer`} onClick={onClick}>
           <div className="flex gap-2">
             ...
             {effectivePosition.includes('left') && (
               ...
               >
                 <svg className="w-full h-full text-gray-500 border-2 border-gray-500 rounded-full p-0.1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* greeting text */}
             <div className="flex-1">
               <div className="font-medium">{effectiveSettings.widget_greeting || widgetText.greeting || "Hey! 👋 I'm Marketrix AI"}</div>
               <div className="text-gray-600">{effectiveSettings.widget_body || widgetText.chat_greeting || "How can I help you?"}</div>
             </div>
           ...
           <div className={`absolute top-1/2 ${effectivePosition.includes('left') ? 'left-full' : 'right-full'} transform -translate-y-1/2 w-0 h-0 border-t-4 border-b-4 ${effectivePosition.includes('left') ? 'border-l-4 border-transparent border-l-white' : 'border-r-4 border-transparent border-r-white'}`} />
         </div>
       )}

      {/* Tooltip - show when welcome text is not displayed */}
      {!isOpen && (!showWelcomeText || effectiveSettings.widget_appearance === 'compact') && (
        <div className={`absolute bottom-16 ${effectivePosition.includes('left') ? 'left-0' : 'right-0'} mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg shadow-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200`}>
          {activeAvatar.name || config.agentName || 'Support Agent'}
          <div className={`absolute top-full ${effectivePosition.includes('left') ? 'left-4' : 'right-4'} w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-gray-900`} />
        </div>
      )}
    </div>
  );
