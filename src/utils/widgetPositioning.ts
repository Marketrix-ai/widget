/**
 * Corner geometry for the widget: the four pinnable corners and the drag-snap math.
 *
 * `getCorner` and `getPanelPositionStyle` place the launcher and panel at a corner, `getResizeGrip`
 * describes the handle opposite it, `getAnchorTopLeft` gives a corner's top-left point,
 * `getReleaseCorner` picks the corner nearest a drag's release plus its flick momentum, and
 * `animateSnap` glides the launcher there and reports when it lands.
 */
...
export const getReleaseCorner = (
  ...
  const x = anchor.x + translation.dx + flick.x;
  const y = anchor.y + translation.dy + flick.y;
