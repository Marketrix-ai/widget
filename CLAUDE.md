...
**Session recording** — `src/services/RrwebSessionRecorder.ts` batches rrweb events and POSTs them via the sdk (`rrweb/metadata` + `rrweb/events`). Requires both `chatId` and `applicationId`. Thresholds: queue cap 500 events, flush at 50 KB queued (`FLUSH_SIZE_THRESHOLD`), batch cap 500 KB (`MAX_BATCH_BYTES`, under the API 5 MB body limit), flush interval 500 ms, drop after 5 consecutive flush failures. `chat_id` is created via `sdk.chatCreate(undefined)` in `ChatSessionManager`. Don't change these without updating the API's expectations.
